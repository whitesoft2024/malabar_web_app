...
const Savings = () => {
    ...
    useEffect(() => {
        fetchSavingsData(currentPage, pageSize,formData.branchCode, searchSavings);
    ...
    const fetchSavingsData = async (
        ...
        try {
            ...
            const response = await axios.get(
                `https://api.malabarbank.in/api/savings?page=${page}&limit=${size}&branchCode=${formData.branchCode}&searchSavings=${searchSavings}&pageNumber=${pageNumber}`
            );
    ...
    const handleSearch = (e) => {
        ...
        fetchSavingsData(1, pageSize, formData.branchCode, value);
    };

    const handleNextPage = () => {
        if (nextPage) {
            setCurrentPage(currentPage + 1);
            fetchSavingsData(currentPage + 1, pageSize,formData.branchCode); // Include branchCode
        }
    };
    ...
    const goToPage = () => {
        ...
        if (!isNaN(pageNumberInt) && pageNumberInt > 0) {
            ...
            fetchSavingsData(pageNumberInt, pageSize, formData.branchCode, searchTerm);
            fetchSavingsData("");
        } else {
