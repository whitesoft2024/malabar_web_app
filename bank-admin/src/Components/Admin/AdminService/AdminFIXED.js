    ...
    const fetchRDSData = async (
        ...
        try {
            ...
            const response = await axios.get(
                `https://api.malabarbank.in/api/fd?page=${page}&limit=${size}&branch=${formData.branchCode}&searchTerm=${searchTerm}`
            );

            const cleanedData = response.data.data.map(({ _id, ...rest }) => rest);
            setNewFDdata(cleanedData);
                            ...
                            <Table striped bordered hover>
                                ...
                                <tbody>
                                    {newFDdata && newFDdata.map((fdData) => (
                                        <tr key={fdData._id} className={fdData ? 'selected-row' : ''}>
                                            <td>{fdData.sl_no}</td>
