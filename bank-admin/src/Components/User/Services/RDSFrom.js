...
const Rds = () => {
  ...
  const updateRDSNumber = (branchCode) => {
    ...
    const newRdsBill = `DRDS${(rdsBillCount + 1).toString().padStart(10, "0")}`;
  ...
  useEffect(() => {
    fetchRDSHistoryData(currentHisPage, hisPageSize, branch, searchTerm, fromDate, toDate);
  }, [currentHisPage, hisPageSize, branch, searchTerm, fromDate, toDate]);

  const fetchRDSHistoryData = async (page, size, branch, searchTerm = "", fromDate = "", toDate = "") => {
    try {
      const response = await fetch(
        `https://api.malabarbank.in/api/RDSdata?page=${page}&limit=${size}&branch=${branch}&searchTerm=${searchTerm}&fromDate=${fromDate}&toDate=${toDate}`
      );
      const data = await response.json();
  ...
  const handleHistorySearch = (e) => {
    const value = e.target.value;
    setSearchTerm(value);
    fetchRDSHistoryData(1, hisPageSize, branch, value, fromDate, toDate);
  };

  const handleNextHistoryPage = () => {
    if (nextHisPage) {
      setCurrentHisPage(currentHisPage + 1);
      fetchRDSHistoryData(currentHisPage + 1, hisPageSize, branch, searchTerm, fromDate, toDate);
    }
  };
  ...
  const handleHistoryPageSizeChange = (e) => {
    ...
    setCurrentHisPage(1);
    fetchRDSHistoryData(1, newSize, branch, searchTerm, fromDate, toDate);
  };

  const handleFetchByDate = () => {
    fetchRDSHistoryData(1, hisPageSize, branch, searchTerm, fromDate, toDate);
  };
  ...
  const goToHisPage = () => {
    const pageNumberInt = parseInt(pageNumber);
    if (!isNaN(pageNumberInt) && pageNumberInt > 0) {
      setCurrentHisPage(pageNumberInt);
      fetchRDSHistoryData(pageNumberInt, hisPageSize, branch, searchTerm, fromDate, toDate);
      setPageNumber("");
    } else {
      const inputBox = document.getElementById("pageNumberInput");
      inputBox.classList.add("input-error");
  ...
  // Initialize the transactionCounter with a default value
  let transactionCounter = 0;
  const generateTransactionId = () => {
    transactionCounter++;
    const paddedCounter = String(transactionCounter).padStart(14, '0');
    return `TRDS${paddedCounter}`;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add the transactionId to formData
      const formDataWithId = { ...formData, transactionId };

      // First API call to save form data
      const response = await axios.post("https://api.malabarbank.in/api/rds", formDataWithId);
  ...
  // };
  const handleRowClickHistory = (item, index, isEmi = false, emiIndex = null) => {
    let balance = 0;

    for (let i = 0; i <= index; i++) {
      const dataItem = historyData[i];

      balance += (dataItem.amount || dataItem.newAmount || 0) - (dataItem.withdrawalAmount || 0);

      if (dataItem.EmiData) {
        for (let j = 0; j < dataItem.EmiData.length; j++) {
          const emiItem = dataItem.EmiData[j];
          balance += (emiItem.newAmount || 0) - (emiItem.withdrawalAmount || 0);

          if (isEmi && i === index && j === emiIndex) break;
        }
      }
    }
    const selectedItemWithBalance = {
      ...item,
      balance: balance
    };
    setSelectedItem(selectedItemWithBalance);
    setShowModalHistory(true); // Open the modal
  };
                      ...
                      <div className="row mb-3">
                        <div className="col-6">
                          ...
                          <input
                            ...
                            onChange={(e) => setFromDate(moment(e.target.value, 'YYYY-MM-DD').format('DD/MM/YYYY'))}
                          />
                        </div>
                        <div className="col-6">
                          ...
                          <input
                            ...
                            onChange={(e) => setToDate(moment(e.target.value, 'YYYY-MM-DD').format('DD/MM/YYYY'))}
                          />
                        </div>
                      </div>
                      <div className="row mb-3">
                        <div className="col-12 text-center">
                          <button className="btn btn-primary" onClick={handleFetchByDate}>Fetch Data</button>
                        </div>
                      </div>
                      ...
                      <center>
                        <Table striped bordered hover>
                            ...
                            {historyData.map((item, index) => (
                              <React.Fragment key={item._id}>
                                ...
                                {item.EmiData && item.EmiData.map((emi, emiIndex) => (
                                  <tr key={`${item._id}_emi_${emiIndex}`} onClick={() => handleRowClickHistory(em)}>
                                    <td>{index + 1}.{emiIndex + 1}</td>
                                    <td>{emi.newDate || emi.Date}</td>
                                    <td>{item.RDSNumber}</td>
                                    <td>{item.customerName}</td>
                                    <td>{emi.Type}</td>
                                    <td>{emi.depositRdsBill || emi.withdrawRdsBill}</td>
                                    <td>
                                      {emi.newAmount || emi.withdrawalAmount}
                                    </td>
                                    <td>{emi.depositTransactionId || emi.withdrawTransactionId}</td>
                                  </tr>
                                ))}
                              </React.Fragment>
                            ))}
                          ...
                          <tbody>
                            {historyData.map((item, index) => (
                              <React.Fragment key={item._id}>
                                <tr onClick={() => handleRowClickHistory(item, index)}>
                                  <td>{index + 1}</td>
                                  <td>{item.newDate || item.date}</td>
                                  <td>{item.RDSNumber}</td>
                                  <td>{item.customerName}</td>
                                  <td>Initial</td>
                                  <td>{item.rdsBill}</td>
                                  <td>{item.amount || item.newAmount || item.withdrawalAmount}</td>
                                  <td>{item.transactionId}</td>
                                </tr>
                                {item.EmiData && item.EmiData.map((emi, emiIndex) => (
                                  <tr key={`${item._id}_emi_${emiIndex}`} onClick={() => handleRowClickHistory(emi, index, true, emiIndex)}>
                                    <td>{index + 1}.{emiIndex + 1}</td>
                                    <td>{emi.newDate || emi.Date}</td>
                                    <td>{item.RDSNumber}</td>
                                    <td>{item.customerName}</td>
                                    <td>{emi.Type}</td>
                                    <td>{emi.depositRdsBill || emi.withdrawRdsBill}</td>
                                    <td>{emi.newAmount || emi.withdrawalAmount}</td>
                                    <td>{emi.depositTransactionId || emi.withdrawTransactionId}</td>
                                  </tr>
                                ))}
                              </React.Fragment>
                        ...
                        <div className="pagination-buttons">
                          ...
                          <nav aria-label="Page navigation example col">
                            <ul class="pagination justify-content-end">
                              ...
                              <li class="page-item">
                                <Button onClick={goToHisPage} disabled={!nextHisPage}>Go</Button>
                              </li>
                            </ul>
                          </nav>
                        </div>
                      ...
                      <div id="receipt-print">
                        <div style={{ border: "1px solid black" }}>
                          ...
                          >
                            ...
                            <p>
                              <b>Transaction ID:</b> {selectedItem.transaction?.id || selectedItem.Type}
                            </p>
                            <p>
                              <b>Date:</b> {selectedItem.transaction?.date || selectedItem.Date || selectedItem.date}
                            </p>
                          </div>
                          ...
                          <div className="ml-5">
                            <p>
                              <b>Received From:</b> {selectedItem.customer?.name || selectedItem.customerName}
                            </p>
                            <p>
                              <b>Address: </b> {selectedItem.customer?.address || selectedItem.address}
                            </p>
                            <p>
                              <b>Account Number: </b> {selectedItem.account?.number || selectedItem.RDSNumber}
                            </p>
                          </div>
                          <Container>
                            <Table striped hover>
                              ...
                              <tbody>
                                <tr>
                                  <td>{selectedItem.transaction?.type || selectedItem.Type || "RDS"}</td>
                                  <td>{selectedItem.transaction?.reference || selectedItem.Type}</td>
                                  <td>{selectedItem.transaction?.description || ''}</td>
                                  <td>{selectedItem.transaction?.amount || selectedItem.amount || selectedItem.newAmount || selectedItem.withdrawalAmount}</td>
                                  <td>{selectedItem.transaction?.balance || selectedItem.balance}</td>
                                </tr>
                              </tbody>
                            </Table>
                          </Container>
                          ...
                          <div className="d-flex">
                            <p className="ml-5">
                              <AmountInWords
                                amount={selectedItem.transaction?.amount || selectedItem.newAmount || selectedItem.withdrawalAmount || selectedItem.withdrawAmount}
                              />
                            </p>
                            <div
                              style={{
                            ...
                            >
                              <p style={{ paddingTop: "5px" }}>
                                <h5>
                                  Total Amount: {selectedItem.balance} /-
                                </h5>
                              </p>
                            </div>
                          </div>
                        ...
                        <div style={{ border: "1px solid black" }}>
                          ...
                          >
                            ...
                            <p>
                              <b>Transaction ID:</b> {selectedItem.transaction?.id || selectedItem.transactionId}
                            </p>
                            <p>
                              <b>Date:</b> {selectedItem.transaction?.date || selectedItem.Date || selectedItem.date}
                            </p>
                          ...
                          <Container>
                            <Table striped hover>
                              ...
                              <tbody>
                                <tr>
                                  <td>{selectedItem.transaction?.type || selectedItem.accountType || "RDS"}</td>
                                  <td>{selectedItem.transaction?.reference || ''}</td>
                                  <td>{selectedItem.transaction?.description || ''}</td>
                                  <td>{selectedItem.transaction?.amount || selectedItem.newAmount || selectedItem.amount || selectedItem.withdrawalAmount}</td>
                          ...
                          <div className="d-flex">
                            <p className="ml-5">
                              <AmountInWords
                                amount={selectedItem.transaction?.amount || selectedItem.newAmount || selectedItem.withdrawalAmount}
                              />
                            </p>
                            <div
                              style={{
                    ...
                    <>
                      <div id="receipt-print">
                        <div style={{ border: "1px solid black" }}>
                          ...
                          >
                            <p><b>Receipt ID:</b> {selectedItem.rdsBill}</p>
                            <p><b>Transaction ID:</b> {selectedItem.transaction?.id || selectedItem.Type}</p>
                            <p><b>Date:</b> {selectedItem.transaction?.date || selectedItem.Date || selectedItem.date}</p>
                          </div>
                          <div style={{ textAlign: "center", marginLeft: "5rem" }}>
                            <img
                              src="/mscs.png"
                          ...
                          <div className="ml-5">
                            <p><b>Received From:</b> {selectedItem.customer?.name || selectedItem.customerName}</p>
                            <p><b>Address: </b> {selectedItem.customer?.address || selectedItem.address}</p>
                            <p><b>Account Number: </b> {selectedItem.account?.number || selectedItem.RDSNumber}</p>
                          </div>
                          <Container>
                            <Table striped hover>
                              <thead>
                                <tr>
                                  <td colSpan={5} style={{ textAlign: "center" }}><b>Information</b></td>
                                </tr>
                                <tr>
                                  <th>Type</th>
                                  <th>Reference</th>
                          ...
                          <div className="d-flex">
                            ...
                            >
                              <p style={{ paddingTop: "5px" }}>
                                <h5>Total Amount: {selectedItem.transaction?.amount || selectedItem.newAmount || selectedItem.withdrawalAmount} /-</h5>
                              </p>
                            </div>
                          </div>
                          <div className="ml-5 d-flex" style={{ marginTop: "5rem" }}>
