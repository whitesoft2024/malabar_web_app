...
const Savings = () => {
  ...
  const handleChange = (e) => {
    const newSavingsBill = "";
    const newSavingsTransactionId = "";
  ...
  const submitFormData = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:2000/api/savings",
        formDataWithoutId
      );
  ...
  const fetchSavingsData = async (
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:2000/api/savings?page=${page}&limit=${size}&branchCode=${branchCode}&searchSavings=${searchSavings}&pageNumber=${pageNumber}`
      );
  ...
  const fetchCustomerData = () => {
    fetch("http://localhost:2000/api/savings")
      .then((response) => {
        if (!response.ok) {
  ...
  const fetchMemberships = async (
    ...
    try {
      setIsLoading(true);
      const response = await axios.get(
        `http://localhost:2000/api/membership?page=${page}&limit=${size}&branchCode=${branchCode}&selectedNumber=${selectedNumber}`
      );
  ...
  const fetchHistoryData = async (
    ...
    try {
      const response = await fetch(
        `http://localhost:2000/api/savings?page=${currentHisPage}&branch=${branch}&limit=${size}&searchTerm=${searchTerm}&fromDate=${fromDate}&toDate=${toDate}`
      );
  ...
  const handleRowClickHistory = (rowData) => {
    setSelectedRowHistory(rowData);
  ...
  const handleSaveChanges = () => {
    ...
    if (confirmSave) {
      axios
        .put(
          `http://localhost:2000/api/savings/${selectedRowEdit.accountNumber}`,
          {
            membershipId,
            customerName,
            accountNumber,
                  ...
                  <div className="form-group d-flex flex-row">
                    ...
                    <div className="col">
                      ...
                      <input
                        ...
                        name="date"
                        className="form-control"
                        value={moment(formData.date, "DD/MM/YYYY").format(
                          "YYYY-MM-DD"
                        )}
            ...
            <div id="receipt-print">
              <div style={{ border: "1px solid black", position: "relative" }}>
                ...
                >
                  ...
                  <p>
                    <b>Transaction ID:</b> {selectedRowHistory.transactionId}
                  </p>
                  <p>
                    <b>Date:</b>{" "}
                    {selectedRowHistory.date || selectedRowHistory.Date}
                  </p>
                </div>
                ...
                <div className="ml-5">
                  <p>
                    <b>Customer Name:</b> {selectedRowHistory.customerName}
                  </p>
                  <p>
                    <b>Phone Number: </b> {selectedRowHistory.customerNumber}
                  </p>
                  <p>
                    <b>Account Number: </b> {selectedRowHistory.accountNumber}
                  </p>
                </div>
                <Container>
                  <Table striped hover>
                    ...
                    <tbody>
                      <tr>
                        <td>
                          {selectedRowHistory.accountType || "Savings Account"}
                        </td>
                        <td>
                          {selectedRowHistory.type || selectedRowHistory.Type}
                        </td>
                        <td>
                          {selectedRowHistory.deposit ||
                            selectedRowHistory.transaction?.amount ||
                            selectedRowHistory.newAmount ||
                            selectedRowHistory.withdrawalAmount}
                        </td>
                        <td>{selectedRowHistory.balanceAmount}</td>
                      </tr>
                    </tbody>
                  </Table>
                </Container>
                <div className="d-flex">
                  <p className="ml-5">
                    {" "}
                    <AmountInWords amount={selectedRowHistory.amount} />
                  </p>
                  <div
                    style={{
                      backgroundColor: "#488a99",
                  ...
                  >
                    <p style={{ paddingTop: "5px" }}>
                      <h5>Total Amount: {selectedRowHistory.amount}/-</h5>
                    </p>
                  </div>
                </div>
              ...
              <div style={{ border: "1px solid black", position: "relative" }}>
                ...
                >
                  <p>
                    <b>Payslip No:</b> {selectedRowHistory.transactionID}
                  </p>
                  <p>
                    <b>Date:</b> {selectedRowHistory.updatedAt}
                  </p>
                </div>
                <div style={{ textAlign: "center", marginLeft: "5rem" }}>
                  <img
                  ...
                  <Table striped hover>
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td>{selectedRowHistory.action}</td>
                        <td>{selectedRowHistory.amount}</td>
                        <td>{selectedRowHistory.balance}</td>
                      </tr>
                    </tbody>
                  </Table>
                </Container>
