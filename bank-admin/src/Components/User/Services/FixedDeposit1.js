...
import { Modal, Button, Popover, Form, Container, } from "react-bootstrap";
...
function FixedDeposit1() {
  ...
  const handleSubmitFd = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("https://api.malabarbank.in/api/fd", formData);
          ...
          <div className="table-container">
            {isLoading ? (
              ...
              <Table striped bordered hover>
                ...
                <tbody>
                  {newFDdata.map((fdData) => (
                    <tr key={fdData._id} onClick={() => handleRowClick(fdData)} className={selectedRow === fdData ? 'selected-row' : ''}>
                      <td>{fdData.sl_no}</td>
                      <td>{fdData.date}</td>
