...
const SavingsWithdrawal = () => {
  ...
  const fetchACCData = async (branch, searchTerm = "") => {
    ...
    response.data.data.forEach((obj) => {
      console.log("Processing account:", obj.accountNumber);
      if (obj.accountNumber === searchTerm) {
        // Check if the deposit field exists and initialize initialDeposit
        if (obj.hasOwnProperty('deposit')) {
          initialDeposit = parseFloat(obj.deposit);
          console.log(`Initial deposit for account ${searchTerm}: ${initialDeposit}`);
        }

        // Initialize sum with initialDeposit
  ...
  const handleSubmit = async (event) => {
    ...
    // const depositAmount = parseFloat(formData.deposit); // Get deposit amount

    // if (totalAmount + depositAmount - withdrawalAmount < 0) {
