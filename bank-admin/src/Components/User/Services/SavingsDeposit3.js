...
const SavingsDeposit = () => {
  ...
  const fetchACCData = async (branch, searchTerm = '') => {
    try {
      setIsLoading(true);
      const response = await axios.get(`http://localhost:2000/api/savings?&branch=${branch}&searchTerm=${searchTerm}`);
...
const handleSubmit = async (event) => {
  ...
  try {
    ...
    const response = await axios.post(`http://localhost:2000/api/savings/addDeposit`, addDeposit);
