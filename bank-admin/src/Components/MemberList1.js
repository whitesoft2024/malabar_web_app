...
function Memberlist() {
    ...
    useEffect(() => {
        fetchMemberships(currentPage, pageSize, branchCode,searchReceipt); 
        console.log(branchCode);
    }, [currentPage, pageSize, branchCode,searchReceipt]);

    const fetchMemberships = async (page, size, branchCode, searchTerm = '', searchReceipt = '') => {
        try {
            setIsLoading(true);
            const response = await axios.get(`https://api.malabarbank.in/api/membership?page=${page}&limit=${size}&branchCode=${branchCode}&searchTerm=${searchTerm}&searchReceipt=${searchReceipt}`);
        ...
        axios.put(`http://localhost:2000/api/membership/${selectedRow._id}`, formData, {
            ...
            .catch(error => {
                ...
                if (error.response && error.response.data) {
                    ...
                    alert(error.response.data.message || 'An error occurred while saving changes');
                } else {
    ...
    const fetchReceiptData = (mobile) => {
        const filteredCustomers = allMemberships.filter(
            ...
            <div className="container border rounded p-4 mt-4 mb-4">
                ...
                <div className="marquee  px-5 m-2">
                    <marquee className="text-white" behavior="scroll" direction="left">
                        New Updates : Welcome to MALABAR CO-OPERATIVE SOCIETY....Have a nice day....
                    </marquee>
                </div>
                <center>
                                    ...
                                    <div className="form-row d-flex flex-row">
                                        ...
                                        <div className="form-group pl-3 col-md-4">
                                            ...
                                            <input
                                                className="form-control"
                                                id="amount"
                                                value={selectedRow ? formData.amount : ''}
                                                onChange={handleInputChange}
                                                name="amount"
                                            />
                                        </div>
                                ...
                                <form className="mt-4">
                                    ...
                                    <div className="row px-2">
                                        <label htmlFor="aadhaar">
                                            AADHAAR CARD NUMBER :
                                            <input className='form-control mt-3' type="text" placeholder="" name="aadharNumber" value={formData.aadharNumber || selectedRow.aadharNumber} onChange={handleInputChange} readOnly/>
                                        </label>
                                    ...
                                    <div className='row px-2'>
                                        ...
                                        <label htmlFor="panCard">
                                            PAN CARD NUMBER :
                                            <input className='form-control mt-3' type="text" name='panNumber' value={formData.panNumber || selectedRow.panNumber} onChange={handleInputChange} readOnly/>
                                        </label>
                                ...
                                <ul className="dropdown-menu2">
                                    {allMemberships
                                        .filter((customer) =>
                                            ...
                                            customer.customerMobile.toLowerCase().includes(searchReceipt.toLowerCase())
                                        )
                                        .map((customer, index) => (
                                            <li key={index} className="dropdown-item" onClick={(e) => { e.stopPropagation(); handleMobileSelect(customer.customerMobile); }}>
